...
public extension CLLocation {
    ...
    var paceMinutesPerKilometer: Double? {
        ...
        return (1000 / speed) / 60.0
    }
}

public extension Array where Element == CLLocation {
    /// Smoothed current pace in minutes per kilometer over the last `windowSeconds`.
    func pace(windowSeconds: TimeInterval = 15) -> Double? {
        guard let last else { return nil }

        let filtered = filter { last.timestamp.timeIntervalSince($0.timestamp) <= windowSeconds }
...
public func formatPace(minPerKm: Double?) -> String {
    guard let minPerKm, minPerKm.isFinite else { return "--:-- /km" }
